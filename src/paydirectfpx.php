...
/**
 * paydirectfpx Payment Gateway Class
 */
class paydirectfpx extends WC_Payment_Gateway {
	function __construct() {
		$this->id = "paydirectfpx";

		$this->method_title = __( "paydirectfpx", 'paydirectfpx' );

		$this->method_description = __( "PayDirectFPX Payment Gateway Plug-in for WooCommerce", 'paydirectfpx' );

		$this->title = __( "paydirectfpx - Pay with internet banking (FPX) or credit / debit cards", 'paydirectfpx' );
		...
		add_action( 'woocommerce_api_'. strtolower( get_class($this) ), array( $this, 'check_paydirectfpx_response' ) );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'        => array(
				'title'   => __( 'Enable / Disable', 'paydirectfpx' ),
				'label'   => __( 'Enable this payment gateway', 'paydirectfpx' ),
				'type'    => 'checkbox',
				'default' => 'no',
			),
			...
			'universal_form' => array(
				'title'    => __( 'Merchant ID', 'paydirectfpx' ),
				'type'     => 'text',
				'desc_tip' => __( 'This is the merchant ID that you can obtain from profile page in PayDirectFPXpx', 'paydirectfpx' ),
			),
			'secretkey'      => array(
				'title'    => __( 'API Key', 'paydirectfpx https://www.paydirect.my https://www.paydirectfpx.com/img/logo.png' ),
				'type'     => 'text',
				'desc_tip' => __( 'This is the API key that you can obtain from profile page in PayDirectFPX profile', 'paydirectfpx' ),
			),
			'order_status' => array(
				 'title' => 'Order Status',
				 'desc_tip' => __( 'Order status to be set after the payment was done. (For Success transaction only)', 'paydirectfpx' ),
				 'type' => 'select',
				 'default' => '1',
				 'label' => 'Label', // checkbox only
				 'options' => array(
			...
			'title' => array(
				'title' => __( 'Title', 'PayDirectFPX' ),
				'type' => 'text',
				'description' => __( 'This controls the title which the user sees during checkout.', 'paydirectfpx' ),
				'default' => __( 'PayDirectFPX - Pay with internet banking (FPX) or credit / debit cards', 'paydirectfpx' ),
				'desc_tip'      => true,
				),
			'description' => array(
			'title' => __( 'Payment Description', 'paydirectfpx' ),
			'type' => 'textarea',
			'default' => ''
			)
		);
					...
					<div id="postbox-container-1" class="postbox-container">
	                        <div id="side-sortables" class="meta-box-sortables ui-sortable">
     							...
     							<div class="postbox ">
	                                ...
	                                <h3 class="hndle"><span align="center">&nbsp;&nbsp;PayDirectFPX <br>
	                                    ...
	                                    <div class="support-widget">
	                                        <ul>
	                                            ...
	                                            <li>» Collect Your Recurring Payments On Time</li>
																					...
																					<a href="https://www.paydirect.my" class="button wpruby_button" target="_blank">Register Now</a>
	                                    </div>
	                                </div>
	                            </div>
	                        </div>
	...
	public function process_payment( $order_id ) {
		...
		return array(
			'result'   => 'success',
			'redirect' => './wc-api/paydirectfpx/?submit_data=true&order_id='.$order_id
		);
	}


	public function check_paydirectfpx_response() {
	    ...
	    // Get an instance of corresponding the WC_Product object
			...
			# Prepare the data to send to paydirectfpx
			...
			{
				echo "<form action='".esc_url("https://www.demo.paydirectfpx.com/woocommerce_receiver.php")."' id='submit_data' method='POST'>";
			}
			else
			{
				echo "<form action='".esc_url("https://www.paydirectfpx.com/woocommerce_receiver.php")."' id='submit_data' method='POST'>";
			}
				...
				$others = number_format( $order_total - $total, 2);
					...
					echo "<tr>
								...
								<tr>z
									...
									<td><small><a href='".esc_url("https://www.paydirect.my")."' target='_blank'><img src='".esc_url("https://www.paydirectfpx.com/img/logo.png")."' width='100'></a></small></td>
								</tr>
								<tr>
						...
						if ( sanitize_text_field($_REQUEST['msg']) == 'Transaction Approved' || sanitize_text_field($_REQUEST['msg']) == 'Success' ) {
							if ( strtolower( $order->get_status() ) == 'pending' || strtolower( $order->get_status() ) == 'processing' ) {
								...
								if ( strtolower( $order->get_status() ) == 'pending' ) {
									...
									$order->add_order_note( 'Payment successfully made through PayDirectFPX ('.sanitize_text_field($_REQUEST['pay_method']).'). Transaction reference is ' . sanitize_text_field($_REQUEST['cart_id']) );
							...
							if ( strtolower( $order->get_status() ) == 'pending' ) {
									...
									add_filter( 'the_content', 'paydirectfpx_payment_declined_msg' );
							}
						}
					} else {
						add_filter( 'the_content', 'paydirectfpx_hash_error_msg' );
					}
				}
